...
export const updateTodo = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        // Jika affected = 0, berarti todo tidak ditemukan atau bukan milik user ini
        if (affectedRows === 0) {
            res.status(404).json({ success: false, message: 'Tugas tidak ditemukan!' });
            return;
        }
...
export const deleteTodo = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        // Jika affectedRows = 0, berati todo tidak ditemuka atau buka milik user ini
        if (affectedRows === 0) {
            res.status(404).json({ success: false, message: 'Tugas tida ditemukan!' });
            return;
        }
